...
add_arg('data_augment_configs', str,    'F:\\个人文件\\大学文件\\开放原子RISC-V\\前后端\\whl\\AudioClassification-Pytorch\\configs\\augmentation.yml', '数据增强配置文件')
add_arg("local_rank",           int,    0,                          '多卡训练需要的参数')
add_arg("use_gpu",              bool,   True,                       '是否使用GPU训练')
add_arg('save_model_path',      str,    'F:\\个人文件\\大学文件\\开放原子RISC-V\\前后端\\whl\\AudioClassification-Pytorch\\model\\',                  '模型保存的路径')
add_arg('log_dir',              str,    'F:\\个人文件\\大学文件\\开放原子RISC-V\\前后端\\whl\\AudioClassification-Pytorch\\log\\',                     '保存VisualDL日志文件的路径')
